<!-- Section Rooms -->
<section class="container section">	
    <div class="row align-items-end">
        <div class="col-12 col-md-12 col-lg-8">
            <span class="title title--overhead js-lines">Rooms</span>
            <h1 class="title title--h1 js-lines">Our Accommodations</h1>
        </div>
        <div class="col-12 col-md-12 col-lg-4 text-lg-right d-none d-md-block">
            <a class="btn-link header-btn-more" href="rooms.html">All</a>
        </div>
    </div>
    
    <!-- Grid -->
    <div class="row">
        <div class="col-12">
            <div class="swiper-container js-rooms">
                <div class="swiper-wrapper">
                    <!-- ItemRoom -->
                    <div class="itemRoom itemRoom__portrait swiper-slide">
                        <span class="badge">Popular</span>
                        <figure class="itemRoom__img-wrap">
                            <a class="itemRoom__link" href="#">
                                <img class="cover" src="assets/images/room_01.jpg" alt="room" />
                            </a>
                        </figure>
                        <div class="itemRoom__details">
                            <h4 class="title title--h4">Standard Suite</h4>
                        </div>	
                    </div>

                    <!-- ItemRoom -->
                    <div class="itemRoom itemRoom__portrait swiper-slide">
                        <figure class="itemRoom__img-wrap">
                            <a class="itemRoom__link" href="#">
                                <img class="cover" src="assets/images/room_02.jpg" alt="room" />
                            </a>
                        </figure>
                        <div class="itemRoom__details">
                            <h4 class="title title--h4">Deluxe Suite</h4>
                        </div>
                    </div>	

                    <!-- ItemRoom -->
                    <div class="itemRoom itemRoom__portrait swiper-slide">
                        <figure class="itemRoom__img-wrap">
                            <a class="itemRoom__link" href="#">
                                <img class="cover" src="assets/images/room_03.jpg" alt="room" />
                            </a>
                        </figure>
                        <div class="itemRoom__details">
                            <h4 class="title title--h4">Family Room Standard</h4>
                        </div>
                    </div>

                    <!-- ItemRoom -->
                    <div class="itemRoom itemRoom__portrait swiper-slide">
                        <figure class="itemRoom__img-wrap">
                            <a class="itemRoom__link" href="#">
                                <img class="cover" src="assets/images/room_04.jpg" alt="room" />
                            </a>
                        </figure>
                        <div class="itemRoom__details">
                            <h4 class="title title--h4">Family Room Garden</h4>
                        </div>
                    </div>
                    
                    <!-- ItemRoom -->
                    <div class="itemRoom itemRoom__portrait swiper-slide">
                        <figure class="itemRoom__img-wrap">
                            <a class="itemRoom__link" href="#">
                                <img class="cover" src="assets/images/room_05.jpg" alt="room" />
                            </a>
                        </figure>
                        <div class="itemRoom__details">
                            <h4 class="title title--h4">Family Room Deluxe</h4>
                        </div>
                    </div>
                </div>
                
                <!-- Pagination  -->
                <div class="swiper-pagination"></div>
            </div>	
        </div>
    </div>			
</section>