<section class="container section section-first" id="contacts">
    <div class="row">
        <div class="col-12">
            <h1 class="title title--h1 js-lines">Find us.</h1>
        </div>
    </div>
    <div class="row">
        <div class="col-12 col-md-6 col-lg-6 col-contact js-scroll-show">
            <h4 class="title title--h4">Address</h4>
            <p class="paragraph">Vila Preá, N/A<br>Preá - Ceará - Brazil</p>
        </div>
        <div class="col-12 col-md-6 col-lg-6 col-contact js-scroll-show">
            <h4 class="title title--h4">Reservations</h4>
            <p class="paragraph">reservas@obahvillage.com.br<br>+55 (88) 9 9664-2583</p>
        </div>
        <div class="col-12 col-md-6 col-lg-6 col-contact js-scroll-show">
            <h4 class="title title--h4">Travel Agencies/Tour Operators</h4>
            <p class="paragraph">comercial@obahvillage.com.br<br>+55 (88) 9 9742-2584</p>
        </div>
        <div class="col-12 col-md-6 col-lg-6 col-contact js-scroll-show">
            <h4 class="title title--h4">Follow us</h4>
            <p class="paragraph">Join us on <a class="link-underline" href="https://instagram.com/pousadaobahvillage_?igshid=MzRlODBiNWFlZA==">Instagram.</a></p>
        </div>
    </div>
</section>